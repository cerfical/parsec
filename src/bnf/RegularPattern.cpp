...
namespace parsec::bnf {
	...
	RegularPattern::RegularPattern(regex::RegularExpr regex, std::string_view name, int id)
		: m_expr(std::move(regex)), m_atoms(FindAllAtoms()(*m_expr.rootNode())), m_name(name), m_id(id)
	{ }
	...
	std::vector<gsl::index> RegularPattern::firstPos() const {
		return ComputeFirstPos()(*expr().rootNode());
	}


	std::vector<gsl::index> RegularPattern::lastPos() const {
		return ComputeLastPos()(*expr().rootNode());
	}
}